...
【改行・余白のルール（絶対厳守）】
- 1つの段落は2〜3文以内にする。それ以上は必ず段落を分ける
- 段落と段落の間には必ず空行を1行入れる
- 見出し（##）の前後には空行を2行入れる
- 内容を詰め込みすぎない。1つのセクションで伝えることは1つに絞る
- スマホで読んだとき、画面いっぱいに文字が詰まっている状態はNG
- 「読みやすさ > 情報量」を徹底する
...
【画像挿入ガイド】
以下の箇所に画像挿入ガイドを明記する：
- 記事冒頭: 📷【アイキャッチ画像】${keyword}に関連する写真
- 症状解説: 📊【図解】${keyword}が起こるメカニズム
- 施術アプローチ: 📷【写真】施術の様子
- セルフケア: 📷【写真】セルフケアの実践写真
...
【改行・読みやすさの指示（絶対厳守）】
- 1つの段落（<p>タグ）は2〜3文以内にしてください。長い段落は絶対に作らない
- 各<p>タグの間には十分な余白が出るように、段落を細かく分けること
- セクション見出し（<h2>）の前後には十分な間隔を持たせること
- 箇条書き（<ul><li>）の各項目は1〜2文で簡潔に
- スマートフォンで読んだとき、画面いっぱいに文字が詰まっている状態はNG
